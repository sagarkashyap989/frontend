...
export const addCart = ({ name, id, image, currentSize, currentQuantity, price, totalPrice}) => async dispatch=>{
    try {
        ...
        // console.log(body)
        const res = await axios.post(`https://sneaker-service.onrender.com/api/cart/${id}`,body, config )
...
export const  cartDelete = (u_id) => async dispatch =>{
    try {
        const res = await axios.delete(`https://sneaker-service.onrender.com/api/cart/${u_id}`)  
